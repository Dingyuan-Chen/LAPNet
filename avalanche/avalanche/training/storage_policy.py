...
class Rev_HerdingSelectionStrategy(FeatureBasedExemplarsSelectionStrategy):
    def make_sorted_indices_from_features(self, features: Tensor) -> List[int]:
        selected_indices = []
...
class InstanceBasedExemplarsSelectionStrategy(ExemplarsSelectionStrategy, ABC):
    """Base class to select exemplars from their features"""
    ...
    def make_sorted_indices(
        ...
        features = []
        for idx, (x, y_true, _) in enumerate(DataLoader(
                data,
                collate_fn=collate_fn,
                batch_size=strategy.eval_mb_size,
        )):
            fea = self.feature_extractor(x.to(strategy.device))
            m = AdaptiveAvgPool2d((fea.size(-2), fea.size(-1)))

            y_true = m(y_true.type(torch.FloatTensor)).unsqueeze(1)
            fea = fea * (y_true < 0.5).to(strategy.device)
...
class Anchor_HerdingSelectionStrategy(InstanceBasedExemplarsSelectionStrategy):
    """The herding strategy as described in iCaRL.
...
__all__ = [
    ...
    "InstanceBasedExemplarsSelectionStrategy",
